...
ToolBox 是一个以 macOS 为主要使用平台的桌面工具启动器，目标是用统一界面管理和快速启动本地工具、脚本、应用和链接。项目最初参考了一款面向安全研究场景的同类产品，但当前仓库的重点不是网络安全工作流本身，而是把它打磨成一个顺手、稳定、可扩展的本地启动器。
